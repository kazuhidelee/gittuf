...
func createTestStateWithPolicy(t *testing.T) *State {
	...
	if err := targetsMetadata.AddRule("protect-main", []string{gpgKey.KeyID}, []string{"git:refs/heads/main"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
	// Add a file protection rule. When used with common.AddNTestCommitsToSpecifiedRef, we have files with names 1, 2, 3,...n.
	if err := targetsMetadata.AddRule("protect-files-1-and-2", []string{gpgKey.KeyID}, []string{"file:1", "file:2"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithPolicyUsingPersons(t *testing.T) *State {
	...
	if err := targetsMetadata.AddRule("protect-main", []string{person.PersonID}, []string{"git:refs/heads/main"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
	// Add a file protection rule. When used with common.AddNTestCommitsToSpecifiedRef, we have files with names 1, 2, 3,...n.
	if err := targetsMetadata.AddRule("protect-files-1-and-2", []string{person.PersonID}, []string{"file:1", "file:2"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithDelegatedPolicies(t *testing.T) *State {
	...
	if err := targetsMetadata.AddRule("1", []string{key.KeyID}, []string{"file:1/*"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}

	if err := targetsMetadata.AddRule("2", []string{key.KeyID}, []string{"file:2/*"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
	...
	if err := delegation1Metadata.AddRule("3", []string{gpgKey.KeyID}, []string{"file:1/subpath1/*"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}

	if err := delegation1Metadata.AddRule("4", []string{gpgKey.KeyID}, []string{"file:1/subpath2/*"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithThresholdPolicy(t *testing.T) *State {
	...
	if err := targetsMetadata.UpdateRule("protect-main", []string{gpgKey.KeyID, approverKey.KeyID}, []string{"git:refs/heads/main"}, 2, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithThresholdPolicyAndGitHubAppTrust(t *testing.T) *State {
	...
	// Set threshold = 2 for existing rule with the added key
	if err := targetsMetadata.UpdateRule("protect-main", []string{person.ID(), approver.ID()}, []string{"git:refs/heads/main"}, 2, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithThresholdPolicyAndGitHubAppTrustForMixedAttestations(t *testing.T) *State {
	...
	if err := targetsMetadata.UpdateRule("protect-main", []string{person.PersonID, approver1.PersonID, approver2.PersonID}, []string{"git:refs/heads/main"}, 3, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithTagPolicy(t *testing.T) *State {
	...
	if err := targetsMetadata.AddRule("protect-tags", []string{gpgKey.KeyID}, []string{"git:refs/tags/*"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
	targetsEnv, err := dsse.CreateEnvelope(targetsMetadata)
	if err != nil {
...
func createTestStateWithThresholdTagPolicy(t *testing.T) *State {
	...
	if err := targetsMetadata.AddRule("protect-tags", []string{gpgKey.KeyID, approverKey.KeyID}, []string{"git:refs/tags/*"}, 2, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
	targetsEnv, err := dsse.CreateEnvelope(targetsMetadata)
	if err != nil {
...
func createTestStateWithTagPolicyForUnauthorizedTest(t *testing.T) *State {
	...
	if err := targetsMetadata.AddRule("protect-tags", []string{rootKey.KeyID}, []string{"git:refs/tags/*"}, 1, tuf.ScopeAll, "", tuf.AccessReadWrite); err != nil {
		t.Fatal(err)
	}
	targetsEnv, err := dsse.CreateEnvelope(targetsMetadata)
	if err != nil {
